...
/* ============================================================================
   ...
   Учитываем только задачи своего бизнес-процесса (src_task_business =
   'Ключевой клиент'): в воронке лежат задачи и других БП — B2C, СберПрайм,
   РКО, оттоки, сервисные по ЮЛ (у task_type 17 разных значений), и контакт
   по ним к контактной политике ключевых клиентов отношения не имеет.
...
where
    --- БТ п.3: только задачи бизнес-процесса «Ключевой клиент»
    task.src_task_business = 'Ключевой клиент'
    --- скобки обязательны: AND связывает сильнее OR, без них фильтр по БП
    --- прилип бы только к is_task_closed
    and (task.is_task_in_progress is true or task.is_task_closed is true)
group by 1, 2, 3
distributed by (inn);
...
/* ============================================================================
   ...
   Роль сопоставляется с воронкой через справочник role_map: коды в воронке
   короче наших (НУ, НО, МКК против НУ ГОСБ, НО ЗП, МКК ТБ).
...
with role_map (eff_role, role_code) as (
    --- Сопоставление наших ролей с role_code воронки задач. Коды в воронке
    --- короче: НУ вместо «НУ ГОСБ», НО вместо «НО ЗП», МКК вместо «МКК ТБ»
    --- (МКК ЦА — отдельный код воронки, к закреплённым ролям не относится).
    --- Для КМ+МЗП контакт смотрим только по МЗП — решение заказчика.
    values
        ('УПР',     'УПР'),
        ('ЗУПР РБ', 'ЗУПР РБ'),
        ('НУ ГОСБ', 'НУ'),
...
src as (
    ...
    from
        tmp_kk_route route
    --- left join, а не inner: если в eff_role появится значение вне справочника,
    --- строка не пропадёт, а просто не найдёт контакт (contact_flag = 1)
    left join
        role_map
            on role_map.eff_role = route.eff_role
    left join
        tmp_kk_contact contact
            ...
            and upper(contact.role_code) = upper(role_map.role_code)
),
...
with slots (eff_role, task_role, task_type) as (
    values
        ...
        ('УПР',     'НУ ГОСБ', 'инфо'),       -- Excel, строка 3   (БТ п.4)
...
Особые случаи: если руководитель компании отказался от встречи или Вы узнали о предстоящем оттоке, начните заполнение комментария к задаче с хэштега #эскалация. Подробно опишите ситуацию: ожидаемая потеря Портфеля ФЛ, должность представителя клиента, дата коммуникации. Проблема будет направлена на уровень ТБ.
{MKK_CA}{TRIGGERS}'),
    -- УПР -> НУ ГОСБ / инфо / привлечение   (Excel Лист2, D3)
    ('УПР', 'НУ ГОСБ', 'инфо', 'привлечение',
     'Компания {COMPANY} ({INN}). ГОСБ №{GOSB}
...
Необходимо обеспечить подготовку Управляющего ГОСБ и закреплённого МЗП ко встрече с клиентом, предоставить уточненные данные по доступному лимиту преференций.{TRIGGERS}'),
    -- УПР -> НУ ГОСБ / инфо / удержание   (Excel Лист2, E3)
    ('УПР', 'НУ ГОСБ', 'инфо', 'удержание',
     'Компания {COMPANY} ({INN}). ГОСБ №{GOSB}
Уважаемый руководитель!
Информируем Вас, что на Управляющего ГОСБ выставлена задача "Стратегия по ключевым клиентам" со сроком отработки до {DUE}.
Необходимо обеспечить подготовку Управляющего ГОСБ и закреплённого МЗП ко встрече с клиентом, предоставить уточненные данные по доступному лимиту преференций.{TRIGGERS}'),
    -- УПР -> НУ ГОСБ / инфо / отток   (Excel Лист2, F3)
    ('УПР', 'НУ ГОСБ', 'инфо', 'отток',
     'Компания {COMPANY} ({INN}). ГОСБ №{GOSB}
Уважаемый руководитель!
