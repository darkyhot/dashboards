...
def reconcile(day: pd.DataFrame, pred: pd.DataFrame, month_elapsed: float) -> pd.DataFrame:
    ...
    Организации без строки в ежедневной витрине наблюдения не имеют — у них остаётся
    полный прогноз (settled = 0). Это две разные ситуации, и обе штатные: выплат в
    этом месяце ещё не было (начало месяца) ЛИБО по последней прошедшей выплате не
    выставлена задача на отток — такие пары `DAY_OUTFLOW` не возвращает вовсе.
    Во втором случае риск месяца у организации не гасится сознательно.
    """
